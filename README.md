...
草野球における **バッター × ピッチャー** および **チーム × チーム** の対戦履歴を記録し、因縁（相性）を「見せたくなるカード」として共有できるアプリです。段階リリースを前提に、初期リリースではローカル記録のみを提供し、その後ログインによるクラウド同期、課金によるチーム共有・因縁生成へ拡張します。
...
| バックエンド | Phase 2以降に Supabase (PostgreSQL + Auth) を導入 |
...
| [MVP1 ローカル特化版設計書](docs/04_MVP1_ローカル特化版設計書.md) | ログインなしで試合・打席/ピッチング成績入力・成績確認を完結させる設計 |
