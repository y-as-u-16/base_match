...
```bash
.agents/skills/issue-worktree/scripts/create_issue_worktrees.sh 123
```

起点ブランチを指定する場合:

```bash
.agents/skills/issue-worktree/scripts/create_issue_worktrees.sh --base develop 123
```

worktree 作成先を指定する場合:

```bash
.agents/skills/issue-worktree/scripts/create_issue_worktrees.sh --root ../worktrees 123
```
