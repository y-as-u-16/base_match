...
MVP1 では Supabase を使わず、端末内の SQLite に自チーム候補、試合、打席、ピッチング成績を保存する。
ローカル版は「自分の端末で記録して、自分の端末で個人成績を見る」ことに特化するため、ユーザー、共有、権限管理のテーブルは持たない。
...
- 選手名: MVP1 では master テーブルを作らず、打席/投球記録ごとに文字列で保存する
- 自チーム: `local_my_teams` で少数の自チーム候補をローカル管理する
- 相手チーム: MVP1 では master テーブルを作らず、試合ごとにチーム名を文字列で保存する
...
| テーブル | 役割 |
|---|---|
| `local_my_teams` | 自分が使うチーム候補を保存する |
| `local_games` | 試合情報を保存する |
| `local_plate_appearances` | 個人別の打席結果を保存する |
| `local_pitching_appearances` | 個人別のピッチング成績を保存する |

### local_my_teams

自分が所属・利用するチーム候補を保存する。
草野球の記録では自チームは 1 個、多くても数個である想定のため、共有やメンバー管理は持たず、試合作成時の選択肢として使う。

| カラム | 型 | null | 備考 |
|---|---|---|---|
| id | text | no | pk。アプリ側で生成した UUID 文字列 |
| name | text | no | 自チーム名 |
| color_key | text | yes | UI でチームを見分けるための色キー |
| is_default | boolean | no | 試合作成時に初期選択するチームかどうか |
| display_order | integer | no | チーム選択肢での並び順 |
| archived_at | datetime | yes | 使わなくなったチームの非表示日時。過去試合との関連は残す |
...
試合の基本情報を保存する。
自チームは `local_my_teams` を参照し、相手チーム名は試合ごとの文字列として保持する。
...
| my_team_id | text | no | `local_my_teams.id` への参照 |
...
個人別の打席結果を保存する。
MVP1 では選手 master を作らず、打者名を文字列として保存する。

| カラム | 型 | null | 備考 |
|---|---|---|---|
| id | text | no | pk。アプリ側で生成した UUID 文字列 |
| game_id | text | no | `local_games.id` への参照 |
| batter_name | text | no | 打者名 |
...
```mermaid
erDiagram
  local_my_teams ||--o{ local_games : plays
...
|---|---|
| users | ログインを扱わないため |
| teams | 共有・招待・メンバー管理を持つクラウド用チームは扱わないため |
| team_members | チーム共有、権限管理を扱わないため |
| players | MVP1 では選手 master を持たず、記録ごとの文字列で十分なため |
...
打撃成績は `local_plate_appearances` から `batter_name` ごとに集計する。
...
ピッチング成績は `local_pitching_appearances` から `pitcher_name` ごとに集計する。
